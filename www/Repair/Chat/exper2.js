...
var testing_prompt = "Type a word to help your partner pick the correct picture then hit Enter.";
...
var prompt_speaker = "Type a word to help your partner pick the correct picture then hit Enter.";

var prompt_speaker_repair = "The other player signalled a problem!!  Type a word to help them pick the correct picture.";

var recieveSpeakersWord_text = "Your partner sent a word. Click the shape that you think they are referring to.";
...
var debrief_URL = "../Debrief_Page.html";
...
setInnerHTML("scorePanel","Score: 0");
...
if(sliderExperiment){
setTimeout(function(){hideMe("SlideWhistle");},1000);
}
...
// iPhone browser displays 'empty image' box when there's no image, so change inner HTML or display property
	...
	if(im=="none"){
		...
		if(im<0){
			// show practice image
			setImageInWindow(stimWindow,practice_ims[-im-1])
			//document.getElementById(stimWindow).src = practice_ims[-im-1];
			}
		else{
			setImageInWindow(stimWindow,ims[im])
		}
...
function setImageInWindow(stimWindow,srcx){
	try{document.getElementById(stimWindow).src = srcx;	}catch(err){}
		try {
				backg = $('#'+stimWindow);
				backg.attr("xlink:href", srcx);
			}catch(err){
			console.log("ERROR IN SETTING IMAGE");
			console.log(err);
			}
}
		...
		if(currentRound == rounds.length){
			...
			if(rounds[currentRound][PARTBREAK] > 0){
				...
				// testing
					...
					testingRound(parseInt(rounds[currentRound][STIMULUS]));
				}
			}
			currentRound += 1;
		}
...
function testingSpeaker1(){
	...
	if(sliderExperiment){
		...
		startSlideWhistle();
		setTimeout('showMe("sendButtonDiv");',2000);
		showMe("retryButton");
		hideMe("PlayPartnerSignal");
	}
	if(sliderExperiment){
...
//	imsIndex = imsIndex.sort(shuffle);
	...
	if(OIRExperiment && !allowRepairInThisExperiment){
		...
		var contextString = rounds[currentRound][CONTEXTIND];
		if(contextString!=""){
			var sx = contextString.split("_");
			stimArray = [];
			for(var i=0;i<sx.length;++i){
				stimArray.push(parseInt(sx[i]));
			}
			shuffle(stimArray);
			if(!containsObject(currentTarget,stimArray)){
				stimArray = stimArray.slice(0,stimArray.length-1);
				stimArray.push(currentTarget);
			}
			// if we don't have enough items
			if(stimArray.length<numberOfContextItems){
				// find meanings not yet in context
				var notIn = [];
				for(var j=0;j< ims.length;++j){
					if(stimArray.indexOf(j) == -1){
						notIn.push(j);
					}
				}
				shuffle(notIn);
				// add meanings to context until it's the right length
				var jx = 0;
				while(stimArray.length<numberOfContextItems){
					stimArray.push(notIn[jx]);
					jx += 1;
					}
			}
		}
...
function clearScreen(){
	...
	//	svg.getElementById("FeedbackPanel").setAttribute("display" , "none");
		...
		hideMe("RepairButton");
		hideMe("BackgroundWaiting");
		hideMe("BackgroundListener");
		hideMe("BackgroundSpeaker");
		hideMe("RepairIndicator");
		hideMe("TrainingBackground");
}
...
function showSpeakerWord(sentWord){
	if(!sliderExperiment){
		setInnerHTML("middleStimTest",sentWord);
...
function myTimer(){
	...
	if(partBreakTimer < 0){
		...
		setInnerHTML("StartButton", click_to_continue_text);
		started = false;
...
function listenerClick(x){
	...
	if(instructionPhase){
		...
		if(waitingForListenerClick){
				...
				waitingForListenerClick = false;
				interactionString += 'RESP='+stimArray[x]+";";
				// remove all images
				if(!cont_visible_after_choice){
					removeImages();
				}	
...
function saveData(){
	// save data after every round  (part1 always saves the data)
...
function loadExperimentFromString(contents){
		console.log("Load experiment from string");
		...
		for(var i=1; i < lines.length -1;++i){  // start from row 1
			...
			if(ix.length>1){
				...
				if(noisInd>=0){
					...
					if(contextInd>=0){
						contextThisRound = ix[contextInd];
					}
					rounds.push([ix[trainingInd],parseInt(ix[stimulusInd]),parseInt(ix[partBreakInd]), ix[messageInd],ix[roleSwitch], ix[pracInd],noiseThisRound,contextThisRound]);
				}
				else{
					// don't include noise or context data
...
//var PRACTICEIND = 5;
	...
	if(rounds[0].length > NOISEIND){
		ret = "Training\tStimulus\tPartBreak\tMessage\tRoleSwitch\tPractice\tNoise\tContext%0A";
	}
	for(var i=0; i<rounds.length;++i){
...
function writeStimOrderFile(){
	console.log("WRITE STIM ORDER FILE");
	console.log(stimOrder);
...
function highlightImage(c){
	try{document.getElementById(c).style.border="thick solid #FF0000";}catch(err){}
}

function dehighlightImage(c){
	try{document.getElementById(c).style.border = "none";}catch(err){}
}
